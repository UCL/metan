...
program define metan, rclass properties(hr nohr shr noshr irr or rrr)
	...
	***********************************************
	...
	syntax [, noPRESERVE noKEEPvars noRSample SAVEDVars /* Added May 2023 */ * ]
	local opts_adm `"`macval(options)'"'
	...
	if `"`savedvars'"'!=`""' & `"`cumulative'`influence'"'==`""' {
		disp `"{error}{bf:savedvars} may only be specified with {bf:cumulative} or {bf:influence}; option will be ignored"'
		local savedvars
	}
	...
	** Create and return matrix of coefficients
	...
	if `"`oldvlist'"'==`""' tokenize `outvlist'				// default
		...
		if `"`savedvars'"'!=`""' {
			...
			// May 2023: for use with `savedvars' option; cumul/infl only
			tempvar _NN2
			qui gen byte `_NN2' = .
			args _ES _seES _LCI _UCI _WT _NN _CC
			tokenize `_ES' `_seES' `_LCI' `_UCI' `_WT' `_NN2' `_CC'
			...
			local extra `extra' cuminfn(`_NN2')		// Default: fill down _NN within metan_output
			if `"`keepvars'"'!=`""' {
				// If nokeepvars, the fill-down routine within metan_output will not be exectuted
				// Hence, instead do it *here* (use subroutine in order to use `sortpreserve')
				FillDown `outvlist', newn(`_NN2') sortby(`sortby' `obs') `opts_adm'
			}
			*/
		}
...
program define ParseModel, sclass
	...
	// Else, assume that `model' is a (valid) model name; continue parsing
		...
		local 0 `", `model'"'
		syntax [, COMMON IVCommon INVVariance IVariance FIXed MHaenszel Random DLaird HMakambi MLe MPaule PMandel PMMedian EBayes CAnova HEdges SEnsitivity DLCommon TSQCommon * ]
		...
		else if inlist("`model'", "dlcommon", "tsqcommon") local model dlc		// DerSimonian-Laird common across subgroups (see Borenstein et al)
		...
		if !inlist("`model'", "sj2s", "dk2s") {
			...
			if "`model'"=="dk2s" {						// DerSimonian-Kacker two-step is valid for MM estimators only
				...
				if !(inlist("`init'", "he", "dl", "dlb", "ev", "hm") /*, "sa"*/ | inlist("`init'", "b0", "bp", "mp", "pmm", "ml", "reml")) {
	...
	** OTHER OPTIONS
	...
	if !`islabel' {
		...
		else if "`model'"=="dlc" local label "DL (Common)"
